...
async def prepare_pipeline(
    ...
    if len(ids) == 0:
        return "Kiinteistörekisteritunnuksia ei tunnistettu! Tarkista syöte."
    try:
        run_pipeline(
            name=uname,
            ids=ids,
        ...
        return "Datan prosessoinnissa tapahtui virhe. Oletko varma, että syötit kiinteistörekisteritunnuksesi oikein?"
    except UserException as e:
        return f"VIRHE: {e}"
    except Exception as e:
        return f"Tapahtui virhe. Odota hetki. VIRHE: {e}"
    return "Metsänhoito-ongelma meställe/metsille " + ", ".join(ids) + " on valmis. "\
           "Paina ylläolevaa siirtyäksesi DESDEO-käyttöliittymään."
...
footer = """
<p id=footer> <a href="https://optgroup.it.jyu.fi/">Monitavoiteoptimointiryhmän kotisivut</p>
"""
...
def response_page(message: str):
    ...
    <html>
        <head>
            <title>Metsäongelman muodostaja</title>
            """ + style + f"""
        </head>
        <body>
            <h1>Viesti palvelimelta: </h1>
            <p>{message}</p>
            """ + back + """
            """ + footer + """
        </body>
...
<html>
    ...
    <body>
        <h1>Metsäongelman muodostaja</h1>
        <p> Luo uusi DESDEO-käyttäjä. </p>
...
def create_user_endpoint(
    ...
    if user is not None:
        return HTMLResponse(response_page(f"Käyttäjä nimellä \"{uname}\" löytyy jo tietokannasta!"))
    ...
    if user is None:
        return HTMLResponse(response_page(f"Käyttäjän \"{uname}\" lisääminen epäonnistui."))

    return HTMLResponse(response_page(f"Käyttäjä \"{uname}\" lisätty tietokantaan."))
...
<html>
    ...
    <body>
        ...
        <p> Syötä DESDEO-käyttäjänimesi ja salasanasi sekä metsäsi kiinteistörekisteritunnus alla oleviin kenttiin. (Ei käyttäjää DESDEOssa? Luo uusi käyttäjä <a href="/create_user">täällä</a>.)
        <p> Tämän jälkeen järjestelmä prosessoi pyyntösi ja muodostaa monitavoiteoptimointiongelman metsäsi pohjalta. Saat ilmoituksen tälle sivulle kun muodostus on valmis. </p>
        <p> <a href=\"""" + desdeo_ui_url +  """/home/\">DESDEO-käyttöliittymään</a> 
        <form action="" onsubmit="sendMessage(event)">
            ...
            <div>
                <label for="forestid">Metsän kiinteistörekisteritunnus (formaatissa 123-123-1234-1234):</label><br>
                <input type="text" name="forestid" id="forestid" value=""/>
            </div>
            <button id="button">Aloita ongelman muodostus</button>
        </form>
        <ul id='messages'>
        </ul>
        <script>
...
async def websocket_endpoint(websocket: WebSocket):
    msg = "Käyttäjän vahvistaminen epäonnistui. Oletko luonut käyttäjän?"
